...
function InputGroup({ id, name, label, placeholder, value, type = "text", register, validation, errors, ...props }) {
  return (
    <>
      {label && (
        <label htmlFor={id} className="block text-sm leading-6 text-white font-bold">
          {label}
        </label>
      )}
      <div className="relative mt-1 rounded-md shadow-sm">
        <input
          ...
          className={`block w-full rounded-md border-0 py-2 text-white shadow-sm ring-1 ring-inset bg-slate-800 ring-white placeholder:text-gray-400 focus:ring-2 focus:ring-inset focus:ring-indigo-600 sm:text-sm sm:leading-6 ${
