...
import { KategoriNewsDetail } from "@/server/admin/kategori-news/kategori-news-detail";
import FormNewsKategori from "@/components/form/form-kategori-news";
...
function EditVacanciesPage({ params }) {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: async () => await KategoriNewsDetail(id),
  });
  if (isLoading) {
    return (
    ...
    <>
      ...
      <div className="overflow-hidden bg-white shadow sm:rounded-lg">
        <div className="px-4 py-5 sm:p-6">
          {/* Content goes here */}
          <FormNewsKategori defaultValues={data.data} isEditing={true} />
        </div>
      </div>
    </>
  );
