...
import { readManyNews } from "@/server/admin/news/read-many-news";
...
import { NewsDelete } from "@/server/admin/news/news-delete";
...
const NewsPage = () => {
  ...
  const canEditNews = true;
  const canDeleteNews = true;
  ...
  const { data, error, isLoading } = useQuery({
    queryKey: ["newss", limit, search, pageParams],
    queryFn: async () =>
      await readManyNews({
        limit,
        search,
      ...
      cell: ({ row }) => (
        <div className="flex space-x-2">
          ...
          <ActionButton
            ...
            onClick={() => {
              router.push("/admin/news/update/" + row.original.id);
            }}
          />
          {/* Delete Button */}
          <ActionButton
